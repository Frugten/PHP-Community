...
?>
<h1>Ret artikler</h1>
<h2>Denne liste viser alle artikler som du har skrevet. Klik på titlen af en artikel For at rette den.</h2>
<p>
<?php
if ($_SESSION['aktiv'] == 1)
{
...
$nquery = mysql_query ("SELECT * FROM artikel WHERE brugernavn ='$bruger'") or die(mysql_error());
...
$query = mysql_query ("SELECT * , DATE_FORMAT(dato, '%d-%m-%Y') AS dato FROM artikel WHERE brugernavn ='$bruger' ORDER BY artikelid DESC limit $vis_fra, $pr_side") or die(mysql_error());
...
echo "$aktiv <a href='$side/artikler/rette.php?menu=$menu&id=$id'>$titel</a> tilføjet $dato ";
echo" <br>";
echo "$beskrivelse<hr>";
}
