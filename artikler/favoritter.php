...
$nquery = mysql_query ("SELECT id FROM favorit WHERE gruppe='artikler' AND bruger = '$bruger'") or die(mysql_error());
...
$nnquery = mysql_query ("SELECT id FROM favorit WHERE gruppe='artikler' AND bruger='$bruger' limit $vis_fra, $pr_side") or die(mysql_error());
while ($b = mysql_fetch_array($nnquery)) 
{
$fav_id = $b[id];
$query = mysql_query ("SELECT * , DATE_FORMAT(dato, '%d-%m-%Y') AS dato FROM artikel WHERE artikelid ='$fav_id' ORDER BY artikelid DESC") or die(mysql_error());
...
echo "<a href='$side/artikler/vis-artikel.php?menu=$menu&id=$id'>$titel</a> tilføjet $dato ";
echo" <br>";
echo "$beskrivelse<br><br>";
}
}
echo "<hr />";
