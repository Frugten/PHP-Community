...
$resultat = mysql_query("SELECT *, DATE_FORMAT(sendt, '%d-%m-%Y') AS dato, DATE_FORMAT(sendt, '%H-%i-%s') AS klok FROM mail_ud WHERE udid= '$id' AND fra = '$bruger'");//Spørger efter ID
$number = mysql_num_rows($resultat);//Tæller antaller af resultater
if ($number == 1)
{
...
echo"<h1>$emne</h1>";
echo"Du sendte denne mail til $til den $dato kl. $klok<br>";
if($laest =="nej")
{
echo"$til har endnu ikke læst denne mail";
}
else
{
echo"$til har læst denne mail";
}
echo"<br><hr>";
echo"$mail<br><br>";
...
{
echo"Du har ikke sendt denne mail. Vælg en af dine egne <a href='udbakke.php?menu=$menu'>her</a>";
}
?>
<?
include 'footer.php';
