...
echo"<h1><a name='svar'>Svar på tråden</a></h1>";

if($_SESSION['prove'] == 1)
{
echo"<b>Sådan vil dit indlæg se ud. For at tilføje det til forummet skal du klikke på send under formularen.</b>";
...
//skal der skiftes side efter tilføj af indlæg
$visning = $_SESSION['forum_indlaeg'];

$naeste_vis = $antal_ideer / $visning;
$naeste_vise = $naeste_vis * $visning;
$h = $vis_fra + $visning;

$_SESSION['page'] = $_SERVER['REQUEST_URI'];
if ($h == $naeste_vise)
{
$vise_fra = $naeste_vise;
$ny_sid= $_SERVER['PHP_SELF'];
$_SESSION['page'] = "$ny_sid?menu=$menu&id=$id&gr=$gr&visfra=$vise_fra";
}
//tjek slut

echo"<table><tr><td>";
echo"<form name='postform' method='POST' ACTION='$side/forum/tilind.php?menu=$menu&visfra=$vise_fra&gr=$gr&traad=$traad'>";
...
$query = mysql_query ("SELECT abbid FROM forum_abb WHERE brugernavn = '$bruger' AND forumgr = '$gr' AND traad ='$traad'") or die(mysql_error());
$abb_antal = mysql_num_rows($query);//Tæller antaller af resultater

if($abb_antal == 1)
{
echo "<input type='checkbox' name='abboner' checked value='ja'> ";
echo"Fjern kryds hvis du ikke længere vil modtage mail når der svares på denne tråd";
}
else
{
if($number == 1)
...
{
echo "<input type='checkbox' name='abboner' value='ja'> ";
}
echo"Vil du modtage en mail når der svares på dennde tråd<br><br>";
}
